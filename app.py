...
if uploaded_file is not None:
    ...
    features = st.sidebar.multiselect("Select Features", edited_df.columns.difference([target_var]))

    if st.sidebar.button("Train Model"):
        try:
            X = edited_df[features]
            ...
            st.success(f"Model trained successfully with an R^2 score of {score:.2f}")
        except Exception as e:
